...
// Rich seeds for realistic 5,000 industrial items generation
...
/**
 * Generate a deterministically rich set of 5,000 real-world inventory items
 */
export function generate5000DummyItems(): InventoryItem[] {
  const items: InventoryItem[] = [];
  const totalCount = 5000;

  // Let's cycle through all categories and generate exactly 5,000 items
  for (let i = 1; i <= totalCount; i++) {
    ...
    // Warehouses & Rack Location distribution
    let warehouse = '';
    let rackLocation = '';
    
    // 8% are unassigned ('미입력') to simulate real-world pending items
    if (i % 12 === 0) {
      warehouse = '미입력';
      rackLocation = '미입력';
    } else {
    ...
    // Quantities & safety stock calculations
    const safetyStock = (i % 6 + 1) * 5; // 5, 10, 15, 20, 25, 30
    let quantity = ((i * 17) % 180) + 1; // 1 to 180
    // Make 10% low stock items for realistic filtering
    if (i % 10 === 0) {
    ...
    // Print status: 65% are printed, 35% are unprinted
    const isPrinted = i % 3 !== 0;
    const printCount = isPrinted ? (i % 4) + 1 : 0;

    // Creation date spread across past 6 months
    const dayOffset = (i % 180);
    const dateObj = new Date(2026, 7, 18); // August 18, 2026
...
// Export 5,000 items as the initial item pool
